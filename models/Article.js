...
class Article {
    ...
    async init() {
        const db = await conection.getBd();
    ...
    async search(term) {
        return this.articles.filter(article => article.title.toLocaleLowerCase().includes(term));
    }

    async sort(articles, sortType = 'asc', sortBy = 'updatedAt') {
        let result
        if (sortType === 'asc') result = await this.#sortAsc(articles, sortBy);
        if (sortType === 'dsc') result = await this.#sortDsc(articles, sortBy);
        return result;
    }
